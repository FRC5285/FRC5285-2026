 ...
 */
public class CommandSwerveDrivetrain extends TunerSwerveDrivetrain implements Subsystem, TelemetryLoggable, ComplexTunable {
    ...
    private final SwerveRequest.ApplyRobotVelocity m_pathApplyRobotSpeeds = new SwerveRequest.ApplyRobotVelocity();
    ...
    public Command fineTunePID(Pose2d goalPose) {
        ...
        .andThen(
            runOnce(() -> 
                this.setControl(
                    this.drivePID.withVelocityX(0.0)
                    .withVelocityY(0.0)
    ...
    public double getVelocityX() {
        // return this.getState().Speeds.vx;
        return this.getState().Velocity.vx * this.getPose().getRotation().getCos()
            - this.getState().Velocity.vy * this.getPose().getRotation().getSin();
    }

    /** Returns Y velocity of robot, Field-Centric, in meters per second */
    public double getVelocityY() {
        // return this.getState().Speeds.vy;
        return this.getState().Velocity.vx * this.getPose().getRotation().getSin()
            + this.getState().Velocity.vy * this.getPose().getRotation().getCos();
    }

    /** Returns rotational velocity of robot, Robot-Centric, in radians per second */
    public double getVelocityRotation() {
        return this.getState().Velocity.omega;
    }
